...
def test_sum_list_of_numbers():
    ...
    assert sum_list_of_numbers([1,2,3]) == 6
...
def test_is_in():
    '''
    assert is_in([1,2,3,4,5], 1)
    assert not is_in([1,2,3,4,5], '1')
    assert is_in(['a','b','c'], 'a')
    '''
    #haystack and needle parameters reversed
...
def test_index_of_number():
    '''
    assert index_of_number([1,2,3,4,5], 1) == 0
    assert index_of_number([1,2,3,4,5], 6) is None
    '''
